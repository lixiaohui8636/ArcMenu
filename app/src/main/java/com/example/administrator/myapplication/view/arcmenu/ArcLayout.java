...
public class ArcLayout extends ViewGroup {
    ...
    public ArcLayout(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mChildSize = Math.max(a.getDimensionPixelSize(R.styleable.ArcLayout_childSize, 600), 60);
    ...
    private void bindChildAnimation( final View child, final int index, final long duration) {
        ...
        Animation animation;
        if(mExpanded){
            animation= createShrinkAnimation(0, toXDelta, 0, toYDelta, startOffset, duration,
                    interpolator);
           ...
           animation= createExpandAnimation(0, toXDelta, 0, toYDelta, startOffset, duration, interpolator);
            animation.setAnimationListener(new AnimationListener() {
    ...
    public void switchState(final boolean showAnimation) {
        if (showAnimation) {
            final int childCount = getChildCount();
            for (int i = 0; i < childCount; i++) {
                bindChildAnimation(getChildAt(i), i, 200);
            }
        }
        Log.i("TAG",""+mExpanded);
        mExpanded = !mExpanded;
